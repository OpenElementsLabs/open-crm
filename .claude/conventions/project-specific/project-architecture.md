...
- **Shared UI package** — Reusable UI components, brand styling, and translations are extracted into `@open-elements/ui` (pnpm workspace package at `frontend/packages/ui/`). The package ships raw `.tsx` source (no build step) — the consuming app compiles it via `transpilePackages` in `next.config.ts` and scans it for Tailwind classes via `@source` directive in `globals.css`. All external dependencies are peer dependencies to avoid version conflicts. TagMultiSelect is backend-agnostic: it receives a `loadTags` callback and `translations` props instead of importing API functions directly.