...
- **Contact Management** — Create, view, edit, and hard-delete contacts with name, email, phone, position, gender, language, and LinkedIn URL. Contacts can be linked to a company via dropdown. Supports filtering by first name/last name/email/company/language, sorting, and pagination. Brevo sync status and double opt-in are displayed as read-only fields in the detail view. The contact list shows a comment count column. The detail view shows comment count in the comments heading. Bidirectional navigation links between contact and company detail views.
