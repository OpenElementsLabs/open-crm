import { de as uiDe } from "@open-elements/ui";

export const de = {
  print: {
    button: "Drucken",
    loading: "Daten werden geladen...",
    noFilters: "Keine Filter",
    noRecords: "Keine Datensätze gefunden",
    filterYes: "Ja",
    filterNo: "Nein",
    filterArchived: "Eingeschlossen",
  },
  app: {
    title: "Open CRM",
    description: "Customer Relationship Management by Open Elements",
    developedBy: "Entwickelt von",
  },
  login: {
    title: "Open CRM",
    button: "Anmelden",
    error: "Anmeldung fehlgeschlagen. Bitte erneut versuchen.",
    developedBy: "Entwickelt von",
  },
  sidebar: {
    uploadAvatar: "Avatar ändern",
    menu: "Menü",
  },
  nav: {
    updates: "Updates",
    search: "Suche",
    companies: "Firmen",
    contacts: "Kontakte",
    opportunities: "Opportunitäten",
    tags: "Tags",
    admin: "Admin",
    serverStatus: "Serverstatus",
    bearerToken: "Bearer Token",
    brevo: "Brevo-Integration",
    enrichment: "Kontakt-Anreicherung",
    webhooks: "Webhooks",
    apiKeys: "API Keys",
    users: "Benutzer",
    auditLogs: "Audit Log",
    backup: "Backup",
  },
  admin: {
    title: "Administration",
    token: {
      title: "Bearer Token",
      show: "Anzeigen",
      hide: "Verbergen",
      copy: "Kopieren",
      copied: "Kopiert",
      validFor: "Gültig für",
      expired: "Abgelaufen",
      noToken: "Kein Token verfügbar",
    },
    capabilities: {
      heic: {
        label: "HEIC-Bilddecodierung",
        available: "Verfügbar",
        unavailable: "Nicht verfügbar",
        hint: "HEIC-Uploads werden mit 415 abgelehnt – Dockerfile prüfen",
      },
    },
  },
  health: {
    title: "Systemstatus",
    statusUp: "Backend läuft",
    statusDown: "Backend ist nicht erreichbar",
  },
  search: {
    title: "Suche",
    inputPlaceholder: "Firmen, Kontakte, Tags und Kommentare durchsuchen…",
    minChars: "Bitte mindestens 2 Zeichen eingeben.",
    emptyResults: "Keine Treffer.",
    bootstrapBanner: "Suchindex wird initialisiert, einen Moment…",
    errorBanner: "Suche ist derzeit nicht verfügbar. Bitte später erneut versuchen.",
    sectionCompanies: "Firmen",
    sectionContacts: "Kontakte",
    sectionTags: "Tags",
    sectionComments: "Kommentare",
    sectionOpportunities: "Opportunitäten",
    showAll: "Alle anzeigen",
  },
  companies: {
    title: "Firmen",
    newCompany: "Neue Firma",
    empty: "Keine Firmen vorhanden. Erstellen Sie die erste Firma.",
    notFound: "Firma nicht gefunden.",
    columns: {
      name: "Name",
      website: "Website",
      contacts: "Kontakte",
      actions: "Aktionen",
    },
    detail: {
      title: "Firmendetails",
      edit: "Bearbeiten",
      delete: "Löschen",
      showEmployees: "Alle Mitarbeiter",
      email: "E-Mail",
      phone: "Telefon",
      website: "Website",
      street: "Straße",
      houseNumber: "Hausnummer",
      zipCode: "PLZ",
      city: "Stadt",
      country: "Land",
      address: "Adresse",
      description: "Beschreibung",
      finance: {
        title: "Finanzen",
        bankName: "Bank",
        bic: "BIC",
        iban: "IBAN",
        vatId: "Umsatzsteuer-ID",
      },
    },
    form: {
      createTitle: "Neue Firma erstellen",
      editTitle: "Firma bearbeiten",
      name: "Name",
      namePlaceholder: "Firmenname eingeben",
      nameRequired: "Name ist erforderlich",
      email: "E-Mail",
      emailPlaceholder: "info@example.com",
      phone: "Telefon",
      phonePlaceholder: "+49 ...",
      website: "Website",
      websitePlaceholder: "https://example.com",
      street: "Straße",
      streetPlaceholder: "Musterstraße",
      houseNumber: "Hausnummer",
      houseNumberPlaceholder: "42",
      zipCode: "PLZ",
      zipCodePlaceholder: "12345",
      city: "Stadt",
      cityPlaceholder: "Berlin",
      country: "Land",
      countryPlaceholder: "Deutschland",
      description: "Beschreibung",
      descriptionPlaceholder: "Firmenbeschreibung eingeben",
      bankName: "Bank",
      bankNamePlaceholder: "z.B. Deutsche Bank",
      bic: "BIC",
      bicPlaceholder: "z.B. DEUTDEDB",
      iban: "IBAN",
      ibanPlaceholder: "z.B. DE89370400440532013000",
      vatId: "Umsatzsteuer-ID",
      vatIdPlaceholder: "z.B. DE123456789",
      financeTitle: "Finanzen",
      logo: "Logo",
      uploadLogo: "Logo hochladen",
      removeLogo: "Logo entfernen",
      save: "Speichern",
      cancel: "Abbrechen",
      errorGeneric: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.",
      imageTooLarge: "Datei ist zu groß (max. 2 MB)",
      imageInvalidFormat: "Ungültiges Dateiformat",
    },
    deleteDialog: {
      title: "Firma löschen",
      descriptionAll: "Firma '{name}' und ALLE zugehörigen Kontakte löschen? Alle Kommentare und Tag-Zuordnungen werden unwiderruflich gelöscht.",
      descriptionOnly: "Nur die Firma '{name}' löschen? Kontakte bleiben erhalten, aber die Firmenzuordnung wird entfernt. Kommentare und Tags der Firma werden gelöscht.",
      deleteAll: "Firma und Kontakte löschen",
      deleteOnly: "Nur Firma löschen",
      cancel: "Abbrechen",
    },
    comments: {
      title: "Kommentare",
      empty: "Keine Kommentare vorhanden",
      add: "Kommentar hinzufügen",
      addTitle: "Kommentar hinzufügen",
      placeholder: "Kommentar eingeben...",
      send: "Senden",
      loadMore: "Mehr laden",
      sending: "Wird gesendet...",
      errorTitle: "Fehler",
      errorGeneric: "Der Kommentar konnte nicht gespeichert werden. Bitte versuchen Sie es erneut.",
      deleteDialog: {
        title: "Kommentar löschen",
        description:
          "Möchten Sie diesen Kommentar wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.",
        confirm: "Löschen",
        cancel: "Abbrechen",
      },
    },
    filter: {
      name: "Nach Name filtern",
    },
    pagination: {
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Firma",
      totalOther: "{count} Firmen",
      perPage: "pro Seite",
    },
  },
  contacts: {
    title: "Kontakte",
    newContact: "Neuer Kontakt",
    empty: "Keine Kontakte vorhanden",
    notFound: "Kontakt nicht gefunden.",
    columns: {
      firstName: "Vorname",
      lastName: "Nachname",
      name: "Name",
      email: "E-Mail",
      company: "Firma",
      actions: "Aktionen",
    },
    detail: {
      title: "Kontaktdetails",
      edit: "Bearbeiten",
      delete: "Löschen",
      firstName: "Vorname",
      lastName: "Nachname",
      email: "E-Mail",
      position: "Position",
      gender: "Geschlecht",
      phone: "Telefon",
      socialLinks: "Soziale Netzwerke",
      language: "Sprache",
      languageDE: "Deutsch",
      languageEN: "Englisch",
      languageUnknown: "Unbekannt",
      birthday: "Geburtstag",
      company: "Firma",
      showCompany: "zur Firma",
      description: "Beschreibung",
      commentsTitle: "Kommentare",
      commentsPlaceholder: "Kommentar hinzufügen",
    },
    form: {
      createTitle: "Neuen Kontakt erstellen",
      editTitle: "Kontakt bearbeiten",
      title: "Titel",
      titlePlaceholder: "z.B. Dr.",
      firstName: "Vorname",
      firstNamePlaceholder: "Vorname eingeben",
      firstNameRequired: "Vorname ist erforderlich",
      lastName: "Nachname",
      lastNamePlaceholder: "Nachname eingeben",
      lastNameRequired: "Nachname ist erforderlich",
      email: "E-Mail",
      emailPlaceholder: "max@example.com",
      position: "Position",
      positionPlaceholder: "z.B. Geschäftsführer",
      gender: "Geschlecht",
      notSpecified: "Nicht angegeben",
      male: "Männlich",
      female: "Weiblich",
      diverse: "Divers",
      socialLinks: "Soziale Netzwerke",
      addSocialLink: "Link hinzufügen",
      networkGithub: "GitHub",
      networkLinkedin: "LinkedIn",
      networkX: "X",
      networkMastodon: "Mastodon",
      networkBluesky: "BlueSky",
      networkDiscord: "Discord",
      networkYoutube: "YouTube",
      networkWebsite: "Webseite",
      placeholderGithub: "Benutzername",
      placeholderLinkedin: "Benutzername oder /in/slug",
      placeholderX: "Benutzername",
      placeholderMastodon: "@benutzer@instanz",
      placeholderBluesky: "handle.bsky.social",
      placeholderDiscord: "numerische Benutzer-ID",
      placeholderYoutube: "@handle",
      placeholderWebsite: "https://example.com",
      phone: "Telefonnummer",
      phonePlaceholder: "+49 ...",
      birthday: "Geburtstag",
      company: "Firma",
      noCompany: "Keine Firma",
      language: "Sprache",
      languageUnknown: "Unbekannt",
      description: "Beschreibung",
      descriptionPlaceholder: "Kontaktbeschreibung eingeben",
      photo: "Foto",
      uploadPhoto: "Foto hochladen",
      removePhoto: "Foto entfernen",
      save: "Speichern",
      cancel: "Abbrechen",
      errorGeneric: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.",
      imageTooLarge: "Datei ist zu groß (max. 2 MB)",
      imageInvalidFormat: "Bitte eine JPEG-, PNG-, WebP- oder HEIC-Datei wählen.",
      managedByBrevo: "Wird von Brevo verwaltet",
    },
    deleteDialog: {
      title: "Kontakt löschen",
      description:
        "Möchten Sie '{name}' wirklich löschen? Alle Kommentare werden unwiderruflich gelöscht.",
      confirm: "Endgültig löschen",
      cancel: "Abbrechen",
    },
    filter: {
      search: "Suche...",
      company: "Nach Firma filtern",
      language: "Nach Sprache filtern",
      allCompanies: "Alle Firmen",
      allLanguages: "Alle Sprachen",
      unknownLanguage: "Unbekannt",
    },
    pagination: {
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Kontakt",
      totalOther: "{count} Kontakte",
      perPage: "pro Seite",
    },
  },
  opportunities: {
    title: "Opportunitäten",
    newOpportunity: "Neue Opportunität",
    empty: "Keine Opportunitäten vorhanden",
    notFound: "Opportunität nicht gefunden.",
    status: {
      open: "offen",
      won: "gewonnen",
      lost: "verloren",
    },
    columns: {
      title: "Titel",
      company: "Firma",
      mainContact: "Hauptkontakt",
      stage: "Phase",
      status: "Status",
      value: "Wert",
      owner: "Verantwortlich",
      actions: "Aktionen",
    },
    filter: {
      search: "Nach Titel suchen...",
      status: "Nach Status filtern",
      allStatuses: "Alle Status",
      stage: "Nach Phase filtern",
      allStages: "Alle Phasen",
      company: "Nach Firma filtern",
      allCompanies: "Alle Firmen",
    },
    detail: {
      title: "Opportunitätsdetails",
      edit: "Bearbeiten",
      delete: "Löschen",
      stage: "Phase",
      product: "Produkt",
      value: "Geschätzter Wert",
      company: "Firma",
      showCompany: "zur Firma",
      mainContact: "Hauptkontakt",
      additionalContacts: "Weitere Kontakte",
      owner: "Verantwortlich",
      createdAt: "Erstellt",
      updatedAt: "Aktualisiert",
    },
    form: {
      createTitle: "Neue Opportunität erstellen",
      editTitle: "Opportunität bearbeiten",
      title: "Titel",
      titlePlaceholder: "z.B. Muster-Bank – CRA Support & Care",
      titleRequired: "Titel ist erforderlich",
      stage: "Phase",
      stagePlaceholder: "Phase wählen oder eingeben",
      status: "Status",
      product: "Produkt",
      productPlaceholder: "Produkt wählen oder eingeben",
      value: "Geschätzter Wert",
      valuePlaceholder: "0,00",
      valueInvalid: "Bitte einen gültigen Betrag mit maximal zwei Nachkommastellen eingeben",
      valueNegative: "Der Wert darf nicht negativ sein",
      company: "Firma",
      companyRequired: "Firma ist erforderlich",
      selectCompany: "Firma wählen",
      mainContact: "Hauptkontakt",
      mainContactRequired: "Hauptkontakt ist erforderlich",
      selectMainContact: "Hauptkontakt wählen",
      additionalContacts: "Weitere Kontakte",
      addContact: "Kontakt hinzufügen",
      owner: "Verantwortlich",
      selectOwner: "Verantwortlichen wählen",
      tags: "Tags",
      contactMismatchWarning: "Kontakt gehört nicht zur ausgewählten Firma",
      save: "Speichern",
      cancel: "Abbrechen",
      errorGeneric: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.",
      noValue: "—",
    },
    deleteDialog: {
      title: "Opportunität löschen",
      description:
        "Möchten Sie '{name}' wirklich löschen? Alle Kommentare werden unwiderruflich gelöscht.",
      confirm: "Endgültig löschen",
      cancel: "Abbrechen",
    },
    pagination: {
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Opportunität",
      totalOther: "{count} Opportunitäten",
      perPage: "pro Seite",
    },
  },
  backup: {
    title: "Backup",
    status: {
      title: "Service-Status",
      notConfigured: "Backup-Service ist nicht konfiguriert.",
      unavailable: "Backup-Service nicht verfügbar.",
      healthy: "Verbunden.",
      infoUnavailable: "Service-Info konnte nicht geladen werden.",
      version: "Version",
      pgDumpVersion: "pg_dump-Version",
      retention: "Aufbewahrung",
      interval: "Intervall",
      lastBackupAge: "Letztes Backup",
    },
    trigger: {
      title: "Manuelles Backup",
      button: "Backup auslösen",
      triggered: "Backup ausgelöst — Job-ID {jobId}",
      alreadyRunning: "Backup läuft bereits — Job-ID {jobId}",
      errorGeneric: "Backup-Service nicht verfügbar.",
    },
    list: {
      title: "Vorhandene Backups",
      columns: {
        createdAt: "Erstellt am",
        sizeBytes: "Größe",
        pgVersion: "pg-Version",
        durationMs: "Dauer",
        triggeredBy: "Trigger",
        sha256: "SHA-256",
        action: "Aktion",
      },
      empty: "Noch keine Backups vorhanden.",
      download: "Herunterladen",
      sha256Copy: "Hash kopieren",
      sha256Copied: "Hash kopiert.",
      errorGeneric: "Backup-Service nicht verfügbar.",
    },
  },
  brevo: {
    title: "Brevo Import",
    settings: {
      title: "API-Einstellungen",
      configured: "API-Key konfiguriert",
      notConfigured: "Kein API-Key konfiguriert",
      apiKeyPlaceholder: "Brevo API-Key eingeben (xkeysib-...)",
      save: "Speichern",
      change: "Ändern",
      remove: "Entfernen",
      saving: "Wird gespeichert...",
      errorInvalid: "Der API-Key ist ungültig. Bitte überprüfen Sie den Key.",
      errorGeneric: "Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.",
      removeConfirm: "API-Key wirklich entfernen?",
    },
    sync: {
      title: "Import",
      start: "Import starten",
      running: "Import läuft... Dies kann einige Minuten dauern.",
      configureFirst: "Bitte konfigurieren Sie zuerst einen API-Key.",
      companiesImported: "Firmen importiert",
      companiesUpdated: "Firmen aktualisiert",
      companiesFailed: "Firmen fehlgeschlagen",
      companiesUnlinked: "Firmen entkoppelt",
      contactsImported: "Kontakte importiert",
      contactsUpdated: "Kontakte aktualisiert",
      contactsFailed: "Kontakte fehlgeschlagen",
      contactsUnlinked: "Kontakte entkoppelt",
      errors: "Fehler",
      noErrors: "Keine Fehler",
      errorConflict: "Ein Import läuft bereits. Bitte warten Sie.",
      errorGeneric: "Der Import konnte nicht gestartet werden.",
    },
  },
  csvExport: {
    button: "CSV",
    title: "CSV Export",
    selectAll: "Alle auswählen",
    deselectAll: "Alle abwählen",
    download: "Download",
    cancel: "Abbrechen",
    companyColumns: {
      name: "Name",
      email: "E-Mail",
      website: "Webseite",
      street: "Straße",
      houseNumber: "Hausnummer",
      zipCode: "PLZ",
      city: "Stadt",
      country: "Land",
      phoneNumber: "Telefon",
      bankName: "Bank",
      bic: "BIC",
      iban: "IBAN",
      vatId: "Umsatzsteuer-ID",
      contactCount: "Kontakte",
      brevo: "Brevo",
      createdAt: "Erstellt am",
      updatedAt: "Aktualisiert am",
    },
    contactColumns: {
      title: "Titel",
      firstName: "Vorname",
      lastName: "Nachname",
      email: "E-Mail",
      position: "Position",
      gender: "Geschlecht",
      socialLinks: "Soziale Netzwerke",
      phoneNumber: "Telefonnummer",
      companyName: "Firma",
      birthday: "Geburtstag",
      language: "Sprache",
      brevo: "Brevo",
      createdAt: "Erstellt am",
      updatedAt: "Aktualisiert am",
    },
  },
  csvImport: {
    button: "Import",
    title: "Kontakte aus CSV importieren",
    cancel: "Abbrechen",
    back: "Zurück",
    next: "Weiter",
    import: "Importieren",
    close: "Schließen",
    steps: {
      upload: "Upload",
      mapping: "Zuordnung",
      preview: "Vorschau",
      result: "Ergebnis",
    },
    upload: {
      description: "CSV-Datei wählen (max. 20 MB, 5.000 Zeilen). Spalten werden im nächsten Schritt zugeordnet.",
      chooseFile: "Datei wählen",
      noFileChosen: "Keine Datei ausgewählt",
      encoding: "Zeichenkodierung",
      hasHeader: "Erste Zeile ist Kopfzeile",
      rowsDetected: "{count} Zeilen erkannt",
    },
    mapping: {
      description: "Ordnen Sie jede CSV-Spalte einem Kontaktfeld zu. Vorname und Nachname sind Pflichtfelder.",
      column: "Quellspalte",
      target: "Zielfeld",
      ignore: "Ignorieren",
      requiredHint: "Vorname und Nachname müssen jeweils genau einmal zugeordnet werden.",
      duplicateHint: "Jedes Zielfeld darf nur einmal zugeordnet werden.",
      targets: {
        TITLE: "Titel",
        FIRST_NAME: "Vorname",
        LAST_NAME: "Nachname",
        EMAIL: "E-Mail",
        POSITION: "Position",
        PHONE_NUMBER: "Telefonnummer",
        LINKEDIN_URL: "LinkedIn-URL",
        WEBSITE_URL: "Website-URL",
      },
    },
    preview: {
      description: "Prüfen Sie die ersten drei zugeordneten Zeilen. Ungültige Felder sind hervorgehoben.",
      row: "Zeile",
      noErrors: "Gültig",
    },
    result: {
      summary: "{created} erstellt, {failed} fehlgeschlagen",
      downloadFailures: "Fehler-CSV herunterladen",
      columns: {
        row: "Zeile",
        field: "Feld",
        reason: "Grund",
      },
    },
    errors: {
      required: "Pflichtfeld",
      invalid: "Ungültig",
      too_long: "Zu lang",
      malformed_row: "Ungültige Zeile",
      save_failed: "Speichern fehlgeschlagen",
      forbidden: "Sie haben keine Berechtigung, Kontakte zu importieren.",
      generic: "Import fehlgeschlagen. Bitte erneut versuchen.",
    },
  },
  vcardExport: {
    button: "vCard",
    single: "vCard herunterladen",
  },
  tags: {
    title: "Tags",
    newTag: "Neuer Tag",
    editTag: "Tag bearbeiten",
    createTag: "Neuen Tag erstellen",
    empty: "Noch keine Tags vorhanden. Erstellen Sie Ihren ersten Tag, um Firmen und Kontakte zu organisieren.",
    createFirst: "Tag erstellen",
    columns: {
      color: "Farbe",
      name: "Name",
      description: "Beschreibung",
      actions: "Aktionen",
    },
    form: {
      name: "Name",
      namePlaceholder: "Tag-Name eingeben",
      nameRequired: "Name ist erforderlich",
      nameConflict: "Ein Tag mit diesem Namen existiert bereits",
      description: "Beschreibung",
      descriptionPlaceholder: "Optionale Beschreibung",
      color: "Farbe",
      colorPlaceholder: "#000000",
      colorRequired: "Farbe ist erforderlich",
      colorInvalid: "Ungültiges Farbformat",
      save: "Speichern",
      cancel: "Abbrechen",
    },
    actions: {
      edit: "Bearbeiten",
      delete: "Löschen",
    },
    deleteDialog: {
      title: "Tag löschen",
      description: "Dieser Tag kann Firmen oder Kontakten zugewiesen sein. Durch das Löschen werden alle Zuweisungen entfernt. Diese Aktion kann nicht rückgängig gemacht werden.",
      errorTitle: "Fehler",
      confirm: "Löschen",
      cancel: "Abbrechen",
    },
    pagination: {
      previous: "Zurück",
      next: "Weiter",
      perPage: "pro Seite",
      totalOne: "{count} Tag",
      totalOther: "{count} Tags",
    },
    label: "Tags",
    selected: "{count} Tags ausgewählt",
  },
  brevoFilter: {
    label: "Brevo",
    all: "Alle",
    fromBrevo: "Aus Brevo",
    notFromBrevo: "Nicht aus Brevo",
  },
  user: {
    logout: "Abmelden",
    noRoles: "Keine Rollen zugewiesen",
  },
  webhooks: {
    title: "Webhooks",
    newWebhook: "Neuer Webhook",
    empty: "Keine Webhooks vorhanden.",
    createFirst: "Webhook erstellen",
    columns: {
      url: "URL",
      active: "Aktiv",
      lastStatus: "Letzter Status",
      lastCalledAt: "Letzter Aufruf",
      actions: "Aktionen",
    },
    status: {
      ok: "OK",
      timeout: "Timeout",
      connectionError: "Verbindungsfehler",
      badCall: "Fehlgeschlagen",
      neverCalled: "—",
    },
    createDialog: {
      title: "Neuen Webhook erstellen",
      urlLabel: "URL",
      urlPlaceholder: "https://example.com/webhook",
      create: "Erstellen",
      cancel: "Abbrechen",
      urlRequired: "URL ist erforderlich",
      error: "Webhook konnte nicht erstellt werden",
    },
    deleteDialog: {
      title: "Webhook löschen",
      description:
        "Möchten Sie diesen Webhook wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.",
      confirm: "Löschen",
      cancel: "Abbrechen",
      errorTitle: "Fehler",
    },
    actions: {
      ping: "Ping senden",
      delete: "Löschen",
      activate: "Aktivieren",
      deactivate: "Deaktivieren",
    },
    pagination: {
      perPage: "pro Seite",
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Webhook",
      totalOther: "{count} Webhooks",
    },
  },
  apiKeys: {
    title: "API Keys",
    newApiKey: "Neuer API Key",
    empty: "Keine API Keys vorhanden.",
    createFirst: "API Key erstellen",
    columns: {
      name: "Name",
      key: "Key",
      createdBy: "Erstellt von",
      createdAt: "Erstellt am",
      actions: "Aktionen",
    },
    createDialog: {
      title: "Neuen API Key erstellen",
      nameLabel: "Name",
      namePlaceholder: "z.B. CI Pipeline",
      create: "Erstellen",
      cancel: "Abbrechen",
      nameRequired: "Name ist erforderlich",
      error: "API Key konnte nicht erstellt werden",
    },
    keyDialog: {
      title: "API Key erstellt",
      warning: "Kopieren Sie diesen Key jetzt. Er wird nicht erneut angezeigt.",
      copy: "Kopieren",
      copied: "Kopiert!",
      close: "Schließen",
    },
    deleteDialog: {
      title: "API Key löschen",
      description:
        "Möchten Sie diesen API Key wirklich löschen? Alle Dienste, die diesen Key verwenden, verlieren sofort den Zugriff.",
      confirm: "Löschen",
      cancel: "Abbrechen",
      errorTitle: "Fehler",
    },
    actions: {
      delete: "Löschen",
    },
    pagination: {
      perPage: "pro Seite",
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} API-Key",
      totalOther: "{count} API-Keys",
    },
  },
  users: {
    title: "Benutzer",
    empty: "Noch keine Benutzer registriert.",
    loadError: "Benutzer konnten nicht geladen werden.",
    columns: {
      avatar: "Avatar",
      name: "Name",
      email: "E-Mail",
    },
    pagination: {
      perPage: "pro Seite",
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Benutzer",
      totalOther: "{count} Benutzer",
    },
  },
  updates: {
    title: "Updates",
    empty: "Noch keine Änderungen",
    loadError: "Updates konnten nicht geladen werden.",
    perPage: "Pro Seite",
    by: "von {user}",
    events: {
      company: {
        created: "Neue Firma {name} wurde angelegt",
        updated: "Firma {name} wurde aktualisiert",
        deleted: "Eine Firma wurde gelöscht",
      },
      contact: {
        created: "Neue Person {name} wurde angelegt",
        updated: "Person {name} wurde aktualisiert",
        deleted: "Eine Person wurde gelöscht",
      },
      companyComment: {
        created: "Firma {name} hat einen neuen Kommentar",
        updated: "Firma {name} hat einen aktualisierten Kommentar",
        deleted: "Kommentar von Firma {name} wurde gelöscht",
      },
      contactComment: {
        created: "Person {name} hat einen neuen Kommentar",
        updated: "Person {name} hat einen aktualisierten Kommentar",
        deleted: "Kommentar von Person {name} wurde gelöscht",
      },
      opportunity: {
        created: "Neue Opportunität {name} wurde angelegt",
        updated: "Opportunität {name} wurde aktualisiert",
        deleted: "Eine Opportunität wurde gelöscht",
      },
      opportunityComment: {
        created: "Opportunität {name} hat einen neuen Kommentar",
        updated: "Opportunität {name} hat einen aktualisierten Kommentar",
        deleted: "Kommentar von Opportunität {name} wurde gelöscht",
      },
    },
  },
  auditLog: {
    title: "Audit Log",
    empty: "Keine Audit-Log-Einträge.",
    loadError: "Audit-Log konnte nicht geladen werden.",
    filter: {
      entityType: "Entity-Typ",
      entityTypeAll: "Alle Typen",
      user: "Benutzer",
      userAll: "Alle Benutzer",
    },
    columns: {
      entityType: "Typ",
      entityId: "Entity-ID",
      action: "Aktion",
      user: "Benutzer",
      createdAt: "Datum",
    },
    pagination: {
      perPage: "pro Seite",
      previous: "Zurück",
      next: "Weiter",
      totalOne: "{count} Eintrag",
      totalOther: "{count} Einträge",
    },
  },
  errors: {
    forbidden: {
      title: "Keine Berechtigung",
      description: "Du hast nicht die erforderliche Rolle für diese Seite.",
      backToHome: "Zur Startseite",
      deleteNoPermission: "Löschen nicht erlaubt — APP-ADMIN-Rolle erforderlich.",
    },
    roleRequired: {
      admin: "APP-ADMIN-Rolle erforderlich",
      itAdmin: "IT-ADMIN-Rolle erforderlich",
    },
  },
  translation: {
    translate: "Übersetzen",
    title: "Übersetzung",
    loading: "Wird übersetzt...",
    error: "Übersetzung fehlgeschlagen. Bitte später erneut versuchen.",
    copy: "Kopieren",
    copied: "Kopiert",
    close: "Schließen",
  },
  enrichment: {
    button: "Anreichern",
    noServices: "Kein Anreicherungsdienst für diesen Kontakt verfügbar",
    services: {
      gravatar: "Gravatar",
      dropcontact: "Dropcontact",
      cognism: "Cognism",
    },
    dialog: {
      title: "Kontakt anreichern über {service}",
      subtitle: "Es werden nur leere Felder gefüllt — nichts wird geändert, bis du bestätigst.",
      loading: "Suche läuft…",
      selectCandidate: "Mehrere Treffer gefunden — bitte einen auswählen:",
      previewTitle: "Die folgenden leeren Felder werden gefüllt:",
      noMatch: "Kein Treffer gefunden.",
      nothingToEnrich: "Nichts anzureichern – alle Felder bereits gefüllt.",
      error: "Anreicherung derzeit nicht möglich.",
      photo: "Profilfoto",
      companyMatched: "Mit bestehender Firma verknüpfen: {name}",
      createCompany: "Neue Firma «{name}» anlegen",
      apply: "Übernehmen",
      cancel: "Abbrechen",
      close: "Schließen",
      success: "Kontakt angereichert.",
    },
    settings: {
      intro: "Konfiguriere die API-Schlüssel für die schlüsselbasierten Anreicherungsdienste. Gravatar benötigt keinen Schlüssel.",
      configured: "API-Schlüssel konfiguriert",
      notConfigured: "Kein API-Schlüssel konfiguriert",
      apiKeyPlaceholder: "API-Schlüssel eingeben",
      save: "Speichern",
      saving: "Wird gespeichert…",
      change: "Ändern",
      remove: "Entfernen",
      removeConfirm: "API-Schlüssel wirklich entfernen?",
      errorInvalid: "Der API-Schlüssel ist ungültig. Bitte den Schlüssel prüfen.",
      errorGeneric: "Ein Fehler ist aufgetreten. Bitte erneut versuchen.",
    },
  },
  pwa: {
    install: "App installieren",
    iosHint: "Zum Installieren: Teilen → Zum Home-Bildschirm",
  },
  ...uiDe,
} as const;
