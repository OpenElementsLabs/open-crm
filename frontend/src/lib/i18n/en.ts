import type { Translations } from "./index";
import { en as uiEn } from "@open-elements/ui";

export const en: Translations = {
  print: {
    button: "Print",
    loading: "Loading data...",
    noFilters: "No filters",
    noRecords: "No records found",
    filterYes: "Yes",
    filterNo: "No",
    filterArchived: "Included",
  },
  app: {
    title: "Open CRM",
    description: "Customer Relationship Management by Open Elements",
    developedBy: "Developed by",
  },
  login: {
    title: "Open CRM",
    button: "Log in",
    error: "Login failed. Please try again.",
    developedBy: "Developed by",
  },
  sidebar: {
    uploadAvatar: "Change avatar",
    menu: "Menu",
  },
  nav: {
    updates: "Updates",
    search: "Search",
    companies: "Companies",
    contacts: "Contacts",
    opportunities: "Opportunities",
    tags: "Tags",
    admin: "Admin",
    serverStatus: "Server Status",
    bearerToken: "Bearer Token",
    brevo: "Brevo Integration",
    enrichment: "Contact Enrichment",
    webhooks: "Webhooks",
    apiKeys: "API Keys",
    users: "Users",
    auditLogs: "Audit Log",
    backup: "Backup",
  },
  admin: {
    title: "Administration",
    token: {
      title: "Bearer Token",
      show: "Show",
      hide: "Hide",
      copy: "Copy",
      copied: "Copied",
      validFor: "Valid for",
      expired: "Expired",
      noToken: "No token available",
    },
    capabilities: {
      heic: {
        label: "HEIC image decoding",
        available: "Available",
        unavailable: "Not available",
        hint: "HEIC uploads will be rejected with 415 — check Dockerfile",
      },
    },
  },
  health: {
    title: "System Status",
    statusUp: "Backend is running",
    statusDown: "Backend is unavailable",
  },
  search: {
    title: "Search",
    inputPlaceholder: "Search across companies, contacts, tags, and comments…",
    minChars: "Type at least 2 characters to search.",
    emptyResults: "No results found.",
    bootstrapBanner: "Search index is initializing, one moment…",
    errorBanner: "Search is currently unavailable. Please try again later.",
    sectionCompanies: "Companies",
    sectionContacts: "Contacts",
    sectionTags: "Tags",
    sectionComments: "Comments",
    sectionOpportunities: "Opportunities",
    showAll: "Show all",
  },
  companies: {
    title: "Companies",
    newCompany: "New Company",
    empty: "No companies found. Create your first company.",
    notFound: "Company not found.",
    columns: {
      name: "Name",
      website: "Website",
      contacts: "Contacts",
      actions: "Actions",
    },
    detail: {
      title: "Company Details",
      edit: "Edit",
      delete: "Delete",
      showEmployees: "show employees",
      email: "Email",
      phone: "Phone",
      website: "Website",
      street: "Street",
      houseNumber: "House Number",
      zipCode: "Zip Code",
      city: "City",
      country: "Country",
      address: "Address",
      description: "Description",
      finance: {
        title: "Finance",
        bankName: "Bank",
        bic: "BIC",
        iban: "IBAN",
        vatId: "VAT ID",
      },
    },
    form: {
      createTitle: "Create New Company",
      editTitle: "Edit Company",
      name: "Name",
      namePlaceholder: "Enter company name",
      nameRequired: "Name is required",
      email: "Email",
      emailPlaceholder: "info@example.com",
      phone: "Phone",
      phonePlaceholder: "+49 ...",
      website: "Website",
      websitePlaceholder: "https://example.com",
      street: "Street",
      streetPlaceholder: "Example Street",
      houseNumber: "House Number",
      houseNumberPlaceholder: "42",
      zipCode: "Zip Code",
      zipCodePlaceholder: "12345",
      city: "City",
      cityPlaceholder: "Berlin",
      country: "Country",
      countryPlaceholder: "Germany",
      description: "Description",
      descriptionPlaceholder: "Enter company description",
      bankName: "Bank",
      bankNamePlaceholder: "e.g. Deutsche Bank",
      bic: "BIC",
      bicPlaceholder: "e.g. DEUTDEDB",
      iban: "IBAN",
      ibanPlaceholder: "e.g. DE89370400440532013000",
      vatId: "VAT ID",
      vatIdPlaceholder: "e.g. DE123456789",
      financeTitle: "Finance",
      logo: "Logo",
      uploadLogo: "Upload logo",
      removeLogo: "Remove logo",
      save: "Save",
      cancel: "Cancel",
      errorGeneric: "An error occurred. Please try again.",
      imageTooLarge: "File is too large (max. 2 MB)",
      imageInvalidFormat: "Invalid file format",
    },
    deleteDialog: {
      title: "Delete Company",
      descriptionAll: "Delete the company '{name}' and ALL associated contacts? All comments and tag associations will be permanently deleted.",
      descriptionOnly: "Delete only the company '{name}'? Contacts will be kept but their company association will be removed. Company comments and tags will be deleted.",
      deleteAll: "Delete company and contacts",
      deleteOnly: "Delete company only",
      cancel: "Cancel",
    },
    comments: {
      title: "Comments",
      empty: "No comments yet",
      add: "Add Comment",
      addTitle: "Add Comment",
      placeholder: "Write a comment...",
      send: "Send",
      loadMore: "Load more",
      sending: "Sending...",
      errorTitle: "Error",
      errorGeneric: "The comment could not be saved. Please try again.",
      deleteDialog: {
        title: "Delete Comment",
        description:
          "Do you really want to delete this comment? This action cannot be undone.",
        confirm: "Delete",
        cancel: "Cancel",
      },
    },
    filter: {
      name: "Filter by name",
    },
    pagination: {
      previous: "Previous",
      next: "Next",
      totalOne: "{count} Company",
      totalOther: "{count} Companies",
      perPage: "per page",
    },
  },
  contacts: {
    title: "Contacts",
    newContact: "New Contact",
    empty: "No contacts found",
    notFound: "Contact not found.",
    columns: {
      firstName: "First Name",
      lastName: "Last Name",
      name: "Name",
      email: "Email",
      company: "Company",
      actions: "Actions",
    },
    detail: {
      title: "Contact Details",
      edit: "Edit",
      delete: "Delete",
      firstName: "First Name",
      lastName: "Last Name",
      email: "Email",
      position: "Position",
      gender: "Gender",
      phone: "Phone",
      socialLinks: "Social Links",
      language: "Language",
      languageDE: "German",
      languageEN: "English",
      languageUnknown: "Unknown",
      birthday: "Birthday",
      company: "Company",
      showCompany: "show company",
      description: "Description",
      commentsTitle: "Comments",
      commentsPlaceholder: "Add Comment",
    },
    form: {
      createTitle: "Create New Contact",
      editTitle: "Edit Contact",
      title: "Title",
      titlePlaceholder: "e.g. Dr.",
      firstName: "First Name",
      firstNamePlaceholder: "Enter first name",
      firstNameRequired: "First name is required",
      lastName: "Last Name",
      lastNamePlaceholder: "Enter last name",
      lastNameRequired: "Last name is required",
      email: "Email",
      emailPlaceholder: "max@example.com",
      position: "Position",
      positionPlaceholder: "e.g. CEO",
      gender: "Gender",
      notSpecified: "Not specified",
      male: "Male",
      female: "Female",
      diverse: "Diverse",
      socialLinks: "Social Links",
      addSocialLink: "Add link",
      networkGithub: "GitHub",
      networkLinkedin: "LinkedIn",
      networkX: "X",
      networkMastodon: "Mastodon",
      networkBluesky: "BlueSky",
      networkDiscord: "Discord",
      networkYoutube: "YouTube",
      networkWebsite: "Website",
      placeholderGithub: "username",
      placeholderLinkedin: "username or /in/slug",
      placeholderX: "username",
      placeholderMastodon: "@user@instance",
      placeholderBluesky: "handle.bsky.social",
      placeholderDiscord: "numeric user ID",
      placeholderYoutube: "@handle",
      placeholderWebsite: "https://example.com",
      phone: "Phone Number",
      phonePlaceholder: "+49 ...",
      birthday: "Birthday",
      company: "Company",
      noCompany: "No company",
      language: "Language",
      languageUnknown: "Unknown",
      description: "Description",
      descriptionPlaceholder: "Enter contact description",
      photo: "Photo",
      uploadPhoto: "Upload photo",
      removePhoto: "Remove photo",
      save: "Save",
      cancel: "Cancel",
      errorGeneric: "An error occurred. Please try again.",
      imageTooLarge: "File is too large (max. 2 MB)",
      imageInvalidFormat: "Please choose a JPEG, PNG, WebP, or HEIC file.",
      managedByBrevo: "Managed by Brevo",
    },
    deleteDialog: {
      title: "Delete Contact",
      description:
        "Do you really want to delete '{name}'? All comments will be permanently deleted.",
      confirm: "Delete permanently",
      cancel: "Cancel",
    },
    filter: {
      search: "Search...",
      company: "Filter by company",
      language: "Filter by language",
      allCompanies: "All companies",
      allLanguages: "All languages",
      unknownLanguage: "Unknown",
    },
    pagination: {
      previous: "Previous",
      next: "Next",
      totalOne: "{count} Contact",
      totalOther: "{count} Contacts",
      perPage: "per page",
    },
  },
  opportunities: {
    title: "Opportunities",
    newOpportunity: "New opportunity",
    empty: "No opportunities yet",
    notFound: "Opportunity not found.",
    status: {
      open: "open",
      won: "won",
      lost: "lost",
    },
    columns: {
      title: "Title",
      company: "Company",
      mainContact: "Main contact",
      stage: "Stage",
      status: "Status",
      value: "Value",
      owner: "Owner",
      actions: "Actions",
    },
    filter: {
      search: "Search by title...",
      status: "Filter by status",
      allStatuses: "All statuses",
      stage: "Filter by stage",
      allStages: "All stages",
      company: "Filter by company",
      allCompanies: "All companies",
    },
    detail: {
      title: "Opportunity details",
      edit: "Edit",
      delete: "Delete",
      stage: "Stage",
      product: "Product",
      value: "Estimated value",
      company: "Company",
      showCompany: "to company",
      mainContact: "Main contact",
      additionalContacts: "Additional contacts",
      owner: "Owner",
      createdAt: "Created",
      updatedAt: "Updated",
    },
    form: {
      createTitle: "Create new opportunity",
      editTitle: "Edit opportunity",
      title: "Title",
      titlePlaceholder: "e.g. Muster-Bank – CRA Support & Care",
      titleRequired: "Title is required",
      stage: "Stage",
      stagePlaceholder: "Select or type a stage",
      status: "Status",
      product: "Product",
      productPlaceholder: "Select or type a product",
      value: "Estimated value",
      valuePlaceholder: "0.00",
      valueInvalid: "Please enter a valid amount with at most two decimal places",
      valueNegative: "The value must not be negative",
      company: "Company",
      companyRequired: "Company is required",
      selectCompany: "Select a company",
      mainContact: "Main contact",
      mainContactRequired: "Main contact is required",
      selectMainContact: "Select a main contact",
      additionalContacts: "Additional contacts",
      addContact: "Add contact",
      owner: "Owner",
      selectOwner: "Select an owner",
      tags: "Tags",
      contactMismatchWarning: "Contact does not belong to the selected company",
      save: "Save",
      cancel: "Cancel",
      errorGeneric: "An error occurred. Please try again.",
      noValue: "—",
    },
    deleteDialog: {
      title: "Delete opportunity",
      description:
        "Do you really want to delete '{name}'? All comments will be permanently deleted.",
      confirm: "Delete permanently",
      cancel: "Cancel",
    },
    pagination: {
      previous: "Previous",
      next: "Next",
      totalOne: "{count} opportunity",
      totalOther: "{count} opportunities",
      perPage: "per page",
    },
  },
  backup: {
    title: "Backup",
    status: {
      title: "Service Status",
      notConfigured: "Backup service is not configured.",
      unavailable: "Backup service is not available.",
      healthy: "Connected.",
      infoUnavailable: "Service info could not be loaded.",
      version: "Version",
      pgDumpVersion: "pg_dump version",
      retention: "Retention",
      interval: "Interval",
      lastBackupAge: "Last backup",
    },
    trigger: {
      title: "Manual backup",
      button: "Trigger backup",
      triggered: "Backup triggered — Job ID {jobId}",
      alreadyRunning: "Backup already running — Job ID {jobId}",
      errorGeneric: "Backup service is not available.",
    },
    list: {
      title: "Existing backups",
      columns: {
        createdAt: "Created at",
        sizeBytes: "Size",
        pgVersion: "pg version",
        durationMs: "Duration",
        triggeredBy: "Trigger",
        sha256: "SHA-256",
        action: "Action",
      },
      empty: "No backups yet.",
      download: "Download",
      sha256Copy: "Copy hash",
      sha256Copied: "Hash copied.",
      errorGeneric: "Backup service is not available.",
    },
  },
  brevo: {
    title: "Brevo Import",
    settings: {
      title: "API Settings",
      configured: "API Key configured",
      notConfigured: "No API Key configured",
      apiKeyPlaceholder: "Enter Brevo API Key (xkeysib-...)",
      save: "Save",
      change: "Change",
      remove: "Remove",
      saving: "Saving...",
      errorInvalid: "The API key is invalid. Please check the key.",
      errorGeneric: "An error occurred. Please try again.",
      removeConfirm: "Really remove API key?",
    },
    sync: {
      title: "Import",
      start: "Start Import",
      running: "Import running... This may take several minutes.",
      configureFirst: "Please configure an API key first.",
      companiesImported: "Companies imported",
      companiesUpdated: "Companies updated",
      companiesFailed: "Companies failed",
      companiesUnlinked: "Companies unlinked",
      contactsImported: "Contacts imported",
      contactsUpdated: "Contacts updated",
      contactsFailed: "Contacts failed",
      contactsUnlinked: "Contacts unlinked",
      errors: "Errors",
      noErrors: "No errors",
      errorConflict: "An import is already running. Please wait.",
      errorGeneric: "The import could not be started.",
    },
  },
  csvExport: {
    button: "CSV",
    title: "CSV Export",
    selectAll: "Select All",
    deselectAll: "Deselect All",
    download: "Download",
    cancel: "Cancel",
    companyColumns: {
      name: "Name",
      email: "Email",
      website: "Website",
      street: "Street",
      houseNumber: "House Number",
      zipCode: "Zip Code",
      city: "City",
      country: "Country",
      phoneNumber: "Phone Number",
      bankName: "Bank",
      bic: "BIC",
      iban: "IBAN",
      vatId: "VAT ID",
      contactCount: "Contacts",
      brevo: "Brevo",
      createdAt: "Created At",
      updatedAt: "Updated At",
    },
    contactColumns: {
      title: "Title",
      firstName: "First Name",
      lastName: "Last Name",
      email: "Email",
      position: "Position",
      gender: "Gender",
      socialLinks: "Social Links",
      phoneNumber: "Phone Number",
      companyName: "Company",
      birthday: "Birthday",
      language: "Language",
      brevo: "Brevo",
      createdAt: "Created At",
      updatedAt: "Updated At",
    },
  },
  csvImport: {
    button: "Import",
    title: "Import Contacts from CSV",
    cancel: "Cancel",
    back: "Back",
    next: "Next",
    import: "Import",
    close: "Close",
    steps: {
      upload: "Upload",
      mapping: "Mapping",
      preview: "Preview",
      result: "Result",
    },
    upload: {
      description: "Choose a CSV file (max 20 MB, 5,000 rows). Columns can be mapped in the next step.",
      chooseFile: "Choose File",
      noFileChosen: "No file chosen",
      encoding: "Encoding",
      hasHeader: "First row is header",
      rowsDetected: "{count} rows detected",
    },
    mapping: {
      description: "Assign each CSV column to a contact field. First name and last name are required.",
      column: "Source column",
      target: "Maps to",
      ignore: "Ignore",
      requiredHint: "Map first name and last name exactly once each.",
      duplicateHint: "Each target can only be mapped once.",
      targets: {
        TITLE: "Title",
        FIRST_NAME: "First name",
        LAST_NAME: "Last name",
        EMAIL: "Email",
        POSITION: "Position",
        PHONE_NUMBER: "Phone number",
        LINKEDIN_URL: "LinkedIn URL",
        WEBSITE_URL: "Website URL",
      },
    },
    preview: {
      description: "Review the first three mapped rows. Invalid fields are highlighted.",
      row: "Row",
      noErrors: "Valid",
    },
    result: {
      summary: "{created} created, {failed} failed",
      downloadFailures: "Download failure CSV",
      columns: {
        row: "Row",
        field: "Field",
        reason: "Reason",
      },
    },
    errors: {
      required: "Required",
      invalid: "Invalid",
      too_long: "Too long",
      malformed_row: "Malformed row",
      save_failed: "Could not save",
      forbidden: "You do not have permission to import contacts.",
      generic: "Import failed. Please try again.",
    },
  },
  vcardExport: {
    button: "vCard",
    single: "Download vCard",
  },
  tags: {
    title: "Tags",
    newTag: "New Tag",
    editTag: "Edit Tag",
    createTag: "Create New Tag",
    empty: "No tags yet. Create your first tag to organize companies and contacts.",
    createFirst: "Create Tag",
    columns: {
      color: "Color",
      name: "Name",
      description: "Description",
      actions: "Actions",
    },
    form: {
      name: "Name",
      namePlaceholder: "Enter tag name",
      nameRequired: "Name is required",
      nameConflict: "A tag with this name already exists",
      description: "Description",
      descriptionPlaceholder: "Optional description",
      color: "Color",
      colorPlaceholder: "#000000",
      colorRequired: "Color is required",
      colorInvalid: "Invalid color format",
      save: "Save",
      cancel: "Cancel",
    },
    actions: {
      edit: "Edit",
      delete: "Delete",
    },
    deleteDialog: {
      title: "Delete Tag",
      description: "This tag may be assigned to companies or contacts. Deleting it removes all assignments. This action cannot be undone.",
      errorTitle: "Error",
      confirm: "Delete",
      cancel: "Cancel",
    },
    pagination: {
      previous: "Previous",
      next: "Next",
      perPage: "per page",
      totalOne: "{count} tag",
      totalOther: "{count} tags",
    },
    label: "Tags",
    selected: "{count} tags selected",
  },
  brevoFilter: {
    label: "Brevo",
    all: "All",
    fromBrevo: "From Brevo",
    notFromBrevo: "Not from Brevo",
  },
  user: {
    logout: "Logout",
    noRoles: "No roles assigned",
  },
  webhooks: {
    title: "Webhooks",
    newWebhook: "New Webhook",
    empty: "No webhooks found.",
    createFirst: "Create Webhook",
    columns: {
      url: "URL",
      active: "Active",
      lastStatus: "Last Status",
      lastCalledAt: "Last Called",
      actions: "Actions",
    },
    status: {
      ok: "OK",
      timeout: "Timeout",
      connectionError: "Connection Error",
      badCall: "Bad Call",
      neverCalled: "—",
    },
    createDialog: {
      title: "Create New Webhook",
      urlLabel: "URL",
      urlPlaceholder: "https://example.com/webhook",
      create: "Create",
      cancel: "Cancel",
      urlRequired: "URL is required",
      error: "Failed to create webhook",
    },
    deleteDialog: {
      title: "Delete Webhook",
      description:
        "Are you sure you want to delete this webhook? This action cannot be undone.",
      confirm: "Delete",
      cancel: "Cancel",
      errorTitle: "Error",
    },
    actions: {
      ping: "Send Ping",
      delete: "Delete",
      activate: "Activate",
      deactivate: "Deactivate",
    },
    pagination: {
      perPage: "per page",
      previous: "Previous",
      next: "Next",
      totalOne: "{count} webhook",
      totalOther: "{count} webhooks",
    },
  },
  apiKeys: {
    title: "API Keys",
    newApiKey: "New API Key",
    empty: "No API keys found.",
    createFirst: "Create API Key",
    columns: {
      name: "Name",
      key: "Key",
      createdBy: "Created By",
      createdAt: "Created",
      actions: "Actions",
    },
    createDialog: {
      title: "Create New API Key",
      nameLabel: "Name",
      namePlaceholder: "e.g. CI Pipeline",
      create: "Create",
      cancel: "Cancel",
      nameRequired: "Name is required",
      error: "Failed to create API key",
    },
    keyDialog: {
      title: "API Key Created",
      warning: "Copy this key now. It will not be shown again.",
      copy: "Copy",
      copied: "Copied!",
      close: "Close",
    },
    deleteDialog: {
      title: "Delete API Key",
      description:
        "Are you sure you want to delete this API key? All services using this key will immediately lose access.",
      confirm: "Delete",
      cancel: "Cancel",
      errorTitle: "Error",
    },
    actions: {
      delete: "Delete",
    },
    pagination: {
      perPage: "per page",
      previous: "Previous",
      next: "Next",
      totalOne: "{count} API key",
      totalOther: "{count} API keys",
    },
  },
  users: {
    title: "Users",
    empty: "No users registered yet.",
    loadError: "Users could not be loaded.",
    columns: {
      avatar: "Avatar",
      name: "Name",
      email: "Email",
    },
    pagination: {
      perPage: "per page",
      previous: "Previous",
      next: "Next",
      totalOne: "{count} User",
      totalOther: "{count} Users",
    },
  },
  updates: {
    title: "Updates",
    empty: "No changes yet",
    loadError: "Failed to load updates.",
    perPage: "Per page",
    by: "by {user}",
    events: {
      company: {
        created: "New company {name} was created",
        updated: "Company {name} was updated",
        deleted: "A company was deleted",
      },
      contact: {
        created: "New person {name} was created",
        updated: "Person {name} was updated",
        deleted: "A person was deleted",
      },
      companyComment: {
        created: "Company {name} has a new comment",
        updated: "Company {name} has an updated comment",
        deleted: "Comment of company {name} was deleted",
      },
      contactComment: {
        created: "Person {name} has a new comment",
        updated: "Person {name} has an updated comment",
        deleted: "Comment of person {name} was deleted",
      },
      opportunity: {
        created: "New opportunity {name} was created",
        updated: "Opportunity {name} was updated",
        deleted: "An opportunity was deleted",
      },
      opportunityComment: {
        created: "Opportunity {name} has a new comment",
        updated: "Opportunity {name} has an updated comment",
        deleted: "Comment of opportunity {name} was deleted",
      },
    },
  },
  auditLog: {
    title: "Audit Log",
    empty: "No audit log entries.",
    loadError: "Audit log could not be loaded.",
    filter: {
      entityType: "Entity Type",
      entityTypeAll: "All types",
      user: "User",
      userAll: "All users",
    },
    columns: {
      entityType: "Type",
      entityId: "Entity ID",
      action: "Action",
      user: "User",
      createdAt: "Date",
    },
    pagination: {
      perPage: "per page",
      previous: "Previous",
      next: "Next",
      totalOne: "{count} Entry",
      totalOther: "{count} Entries",
    },
  },
  errors: {
    forbidden: {
      title: "Access denied",
      description: "You don't have the required role to access this page.",
      backToHome: "Back to home",
      deleteNoPermission: "Delete not allowed — APP-ADMIN role required.",
    },
    roleRequired: {
      admin: "APP-ADMIN role required",
      itAdmin: "IT-ADMIN role required",
    },
  },
  translation: {
    translate: "Translate",
    title: "Translation",
    loading: "Translating...",
    error: "Translation failed. Please try again later.",
    copy: "Copy",
    copied: "Copied",
    close: "Close",
  },
  enrichment: {
    button: "Enrich",
    noServices: "No enrichment service available for this contact",
    services: {
      gravatar: "Gravatar",
      dropcontact: "Dropcontact",
      cognism: "Cognism",
    },
    dialog: {
      title: "Enrich contact via {service}",
      subtitle: "Only empty fields are filled — nothing is changed until you confirm.",
      loading: "Searching…",
      selectCandidate: "Several matches were found — please choose one:",
      previewTitle: "The following empty fields will be filled:",
      noMatch: "No match found.",
      nothingToEnrich: "Nothing to enrich — all fields are already filled.",
      error: "Enrichment is currently not possible.",
      photo: "Profile photo",
      companyMatched: "Link to existing company: {name}",
      createCompany: "Create new company «{name}»",
      apply: "Apply",
      cancel: "Cancel",
      close: "Close",
      success: "Contact enriched.",
    },
    settings: {
      intro: "Configure the API keys for the key-based enrichment services. Gravatar needs no key.",
      configured: "API key configured",
      notConfigured: "No API key configured",
      apiKeyPlaceholder: "Enter API key",
      save: "Save",
      saving: "Saving…",
      change: "Change",
      remove: "Remove",
      removeConfirm: "Really remove the API key?",
      errorInvalid: "The API key is invalid. Please check the key.",
      errorGeneric: "An error occurred. Please try again.",
    },
  },
  pwa: {
    install: "Install app",
    iosHint: "To install: Share → Add to Home Screen",
  },
  ...uiEn,
} as const;
