...
- [ ] Create `backend/src/main/resources/db/migration/V6__make_language_nullable.sql`
  - `ALTER TABLE contacts ALTER COLUMN language DROP NOT NULL;`

**Acceptance criteria:**
- [ ] Project builds successfully
- [ ] Existing contacts retain their language values
...
- [ ] In `ContactEntity.java`: change `@Column(name = "language", nullable = false, length = 5)` to `nullable = true`
- [ ] In `ContactEntity.java`: remove `Objects.requireNonNull` from `setLanguage()`, allow null
- [ ] In `ContactCreateDto.java`: remove `@NotNull` annotation from `language`, change schema to `NOT_REQUIRED`
- [ ] In `ContactUpdateDto.java`: remove `@NotNull` annotation from `language`, change schema to `NOT_REQUIRED`
- [ ] In `ContactDto.java`: change schema from `REQUIRED` to `NOT_REQUIRED` for `language`
- [ ] Update Javadoc in DTOs to reflect nullable language
...
- [ ] In `ContactController.java`: change `language` param type from `Language` to `String` to support a "UNKNOWN" filter value
- [ ] In `ContactService.java`: update list method to accept `String` for language, handle "UNKNOWN" → `IS NULL` filter, parse DE/EN as before
...
- [ ] Add tests to `ContactControllerTest`:
...
- [ ] In `types.ts`: change `language: "DE" | "EN"` to `language: "DE" | "EN" | null` in `ContactDto`
- [ ] In `types.ts`: change `language: "DE" | "EN"` to `language?: "DE" | "EN" | null` in `ContactCreateDto`
- [ ] In `de.ts`: add `contacts.form.unknown: "Unbekannt"` and `contacts.filter.unknown: "Unbekannt"`
- [ ] In `en.ts`: add `contacts.form.unknown: "Unknown"` and `contacts.filter.unknown: "Unknown"`
...
- [ ] In `contact-form.tsx`: remove required asterisk from language label
- [ ] Remove language validation error logic (no longer required)
- [ ] Add "Unbekannt"/"Unknown" option to language Select that maps to null
- [ ] In edit mode, show "Unbekannt"/"Unknown" when contact.language is null
- [ ] In create mode, no pre-selected value

**Acceptance criteria:**
- [ ] Frontend builds successfully
- [ ] Language field is optional in create/edit forms
...
- [ ] In `contact-detail.tsx`: when `contact.language` is null, display "Unbekannt" (DE) or "Unknown" (EN) instead of raw value

**Acceptance criteria:**
- [ ] Frontend builds successfully
- [ ] Null language shown as translated "Unknown"
...
- [ ] In `contact-list.tsx`: add "Unbekannt"/"Unknown" option to language filter dropdown
- [ ] Map the "Unknown" filter value to a query parameter that the backend interprets as IS NULL

**Acceptance criteria:**
- [ ] Frontend builds successfully
- [ ] Unknown language filter option available
...
- [ ] Update `contact-form.test.tsx`:
  - "should not require language" — submit without language selection, expect success
  - "should show Unknown option in language dropdown"
  - "should show Unknown as selected when editing contact with null language"
- [ ] Update `contact-detail.test.tsx`:
  - "should display Unbekannt when language is null"
  - "should display DE when language is DE" (existing behavior, may need mock update)
- [ ] Update `contact-list.test.tsx`:
  - "should show Unknown option in language filter"
...
- [ ] Update `.claude/conventions/project-specific/project-features.md` — note optional language field
